...
@server.tool()
@handle_http_errors("delete_chat_message", service_type="chat")
@require_google_service("chat", "chat")
async def delete_chat_message(
    ...
    Args:
        ...
        space_name (str): The name of the space (format: "spaces/XXX"). Required.
        message_id (str): The ID of the message to delete. Required.

    Returns:
        str: Confirmation message of the deletion.
    """
    logger.info(
        f"[delete_chat_message] Invoked. Email: '{user_google_email}', Space: '{space_name}', Message: '{message_id}'"
    )

    await asyncio.to_thread(
        service.spaces()
        .messages()
        .delete(name=f"{space_name}/messages/{message_id}")
        .execute
    )

    confirmation_message = (
        f"Message deleted successfully from space '{space_name}' for {user_google_email}. "
        f"Message ID: {message_id}"
    )

    logger.info(f"Message deleted successfully for {user_google_email}")
...
async def update_chat_message(
    ...
    """
    Updates (edits) a message in a Google Chat space.

    Args:
        user_google_email (str): The user's Google email address. Required.
        space_name (str): The name of the space (format: "spaces/XXX"). Required.
        message_id (str): The ID of the message to update. Required.
        text (str): The new text content for the message. Required.

    Returns:
        str: Confirmation message with updated message details.
    """
    logger.info(
        f"[update_chat_message] Invoked. Email: '{user_google_email}', Space: '{space_name}', Message: '{message_id}'"
    )
    ...
    message_text = updated_message.get("text", "")[:100]
    create_time = updated_message.get("createTime", "Unknown")

    confirmation_message = (
        f"Message updated successfully in space '{space_name}' for {user_google_email}. "
        f"Message ID: {message_id}, Created: {create_time}"
    )

    logger.info(f"Message updated successfully for {user_google_email}")
    return confirmation_message
