...
async def move_drive_file(
    ...
    remove_from_current_parent: bool = True,
) -> str:
    """
    ...
    Args:
        ...
        new_parent_folder_id (str): The ID of the destination folder. Required.
        remove_from_current_parent (bool): Whether to remove the file from its current parent.
            Defaults to True. If False, the file will be copied to the new location instead.

    Returns:
        str: Confirmation message with the file's new location.
    """
    logger.info(
        f"[move_drive_file] Invoked. Email: '{user_google_email}', File: {file_id}, "
        f"New parent: {new_parent_folder_id}"
    )
    ...
    text_output = (
        f"Successfully {action} file '{file_name}' (ID: {file_id}) "
        f"to folder '{new_parent_folder_id}' for {user_google_email}."
    )

    logger.info(f"Successfully {action} file for {user_google_email}.")
    return text_output
...
async def delete_drive_file(
    ...
    Args:
        ...
        trash (bool): If True, moves the file to trash. If False, permanently deletes the file.
            Defaults to True (trash). Note: Permanently deleted files cannot be recovered.

    Returns:
        str: Confirmation message of the deletion.
    """
    logger.info(
        f"[delete_drive_file] Invoked. Email: '{user_google_email}', File: {file_id}, "
        f"Trash: {trash}"
    )
    ...
    text_output = f"Successfully {action} file '{file_name}' (ID: {file_id}) for {user_google_email}."
...
@server.tool()
@handle_http_errors("get_drive_file_revisions", is_read_only=True, service_type="drive")
@require_google_service("drive", "drive_read")
async def get_drive_file_revisions(
    service,
    user_google_email: str,
    file_id: str,
    page_size: int = 50,
    ...
    """
    Gets the revision history for a file in Google Drive.

    Args:
        user_google_email (str): The user's Google email address. Required.
        file_id (str): The ID of the file. Required.
        page_size (int): Maximum number of revisions to return. Defaults to 50.

    Returns:
        str: Formatted list of file revisions with details.
    """
    logger.info(
        f"[get_drive_file_revisions] Invoked. Email: '{user_google_email}', File: {file_id}"
    )
    ...
    revisions = await asyncio.to_thread(
        ...
        .list(
            fileId=file_id,
            pageSize=page_size,
            fields="revisions(id,modifiedTime,lastModifyingUser,exportLink,originalFilename)",
            supportsAllDrives=True,
        )
        .execute
    )
    ...
    if not revision_list:
        return f"No revisions found for file '{file_name}' (ID: {file_id})."

    output_parts = [
        f"Revisions for '{file_name}' (ID: {file_id}):",
        f"Total revisions: {len(revision_list)}",
        "",
    ]

    for i, rev in enumerate(revision_list, 1):
        ...
        output_parts.append(f"Rev {i} (ID: {rev_id}):")
        output_parts.append(f"  Modified: {modified_time}")
        output_parts.append(f"  By: {user_name}")

        if is_google_doc:
            export_link = rev.get("exportLink")
            if export_link:
                output_parts.append(f"  Export: {export_link}")
...
async def restore_drive_revision(
    ...
    """
    Restores a file to a previous revision in Google Drive.
    This creates a new revision with the content from the specified revision.

    Args:
        user_google_email (str): The user's Google email address. Required.
        file_id (str): The ID of the file. Required.
        revision_id (str): The ID of the revision to restore. Required.

    Returns:
        str: Confirmation message of the restore operation.
    """
    logger.info(
        f"[restore_drive_revision] Invoked. Email: '{user_google_email}', "
        f"File: {file_id}, Revision: {revision_id}"
    )
    ...
    if is_google_doc:
        ...
        if export_mime:
            export_link = (
                f"https://docs.google.com/feeds/download/documents/export/Export?id={file_id}"
                f"&revision={revision_id}&exportFormat=docx"
            )

            async with httpx.AsyncClient() as client:
                response = await client.get(export_link)
            ...
            if response.status_code == 200:
                ...
                text_output = (
                    f"Successfully restored file '{file_name}' (ID: {file_id}) "
                    f"to revision {revision_id} from {modified_time} by {user_name}. "
                    f"New revision created."
                )
            else:
                raise UserInputError(
                    f"Failed to export revision: HTTP {response.status_code}"
                )
    else:
        revision_content = await asyncio.to_thread(
            service.revisions()
            .get_media(
                fileId=file_id,
                revisionId=revision_id,
    ...
    logger.info(f"Successfully restored revision for {user_google_email}.")
    return text_output
