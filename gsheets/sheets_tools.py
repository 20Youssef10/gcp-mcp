...
@server.tool()
@handle_http_errors("copy_spreadsheet", is_read_only=True, service_type="sheets")
...
async def copy_spreadsheet(
    ...
    """
    Copies an existing spreadsheet to create a new one.

    Args:
        user_google_email (str): The user's Google email address. Required.
        spreadsheet_id (str): The ID of the spreadsheet to copy. Required.
        title (str): The title for the new copied spreadsheet. Required.

    Returns:
        str: Confirmation message with the new spreadsheet ID and URL.
    """
    logger.info(
        f"[copy_spreadsheet] Invoked. Email: '{user_google_email}', Source: {spreadsheet_id}, New title: {title}"
    )
    ...
    text_output = (
        f"Successfully copied spreadsheet to '{title}' for {user_google_email}. "
        f"ID: {new_id} | URL: {new_url}"
    )

    logger.info(
        f"Successfully copied spreadsheet for {user_google_email}. New ID: {new_id}"
    )
    return text_output
...
async def delete_sheet(
    ...
    """
    logger.info(
        f"[delete_sheet] Invoked. Email: '{user_google_email}', Spreadsheet: {spreadsheet_id}, Sheet: {sheet_name}"
    )
    ...
    text_output = f"Successfully deleted sheet '{sheet_name}' (ID: {sheet_id}) from spreadsheet {spreadsheet_id} for {user_google_email}."
...
@server.tool()
@handle_http_errors("freeze_rows_columns", service_type="sheets")
@require_google_service("sheets", "sheets_write")
async def freeze_rows_columns(
    ...
    Args:
        user_google_email (str): The user's Google email address. Required.
        spreadsheet_id (str): The ID of the spreadsheet. Required.
        sheet_name (str): Name of the sheet. Defaults to first sheet if not provided.
        num_rows (int): Number of rows to freeze. Set to 0 to unfreeze rows. Defaults to 0.
        num_columns (int): Number of columns to freeze. Set to 0 to unfreeze columns. Defaults to 0.

    Returns:
        str: Confirmation message of the freeze operation.
    """
    logger.info(
        f"[freeze_rows_columns] Invoked. Email: '{user_google_email}', Spreadsheet: {spreadsheet_id}, "
    ...
    if num_rows < 0 or num_columns < 0:
        raise UserInputError("num_rows and num_columns must be non-negative.")

    fields_to_update = []
    grid_properties = {}

    if num_rows > 0:
        grid_properties["frozenRowCount"] = num_rows
        fields_to_update.append("gridProperties.frozenRowCount")

    if num_columns > 0:
        grid_properties["frozenColumnCount"] = num_columns
        fields_to_update.append("gridProperties.frozenColumnCount")
    ...
    parts = []
    if num_rows > 0:
        parts.append(f"{num_rows} row(s)")
    else:
        parts.append("rows unfrozen")
    if num_columns > 0:
        parts.append(f"{num_columns} column(s)")
    else:
        parts.append("columns unfrozen")

    text_output = (
        f"Successfully updated freeze settings for sheet '{sheet_title}' in spreadsheet "
        f"{spreadsheet_id} for {user_google_email}: {', '.join(parts)}."
    )

    logger.info(f"Successfully updated freeze settings for {user_google_email}.")
    return text_output
...
async def merge_cells(
    ...
    """
    Merges cells in a range together into a single cell.

    Args:
        user_google_email (str): The user's Google email address. Required.
        spreadsheet_id (str): The ID of the spreadsheet. Required.
        range_name (str): The A1 range to merge (e.g., "A1:D1"). Required.
        merge_type (str): The type of merge. Options: "MERGE_ALL" (merges all cells into one),
            "MERGE_ROWS" (merges cells in each row independently),
            "MERGE_COLUMNS" (merges cells in each column independently). Defaults to "MERGE_ALL".

    Returns:
        str: Confirmation message of the merge operation.
    """
    allowed_types = {"MERGE_ALL", "MERGE_ROWS", "MERGE_COLUMNS"}
    merge_type_normalized = merge_type.upper()
    if merge_type_normalized not in allowed_types:
        raise UserInputError(
            f"merge_type must be one of {sorted(allowed_types)}, got '{merge_type}'."
        )

    logger.info(
        f"[merge_cells] Invoked. Email: '{user_google_email}', Spreadsheet: {spreadsheet_id}, "
        f"Range: {range_name}, Type: {merge_type_normalized}"
    )
    ...
    text_output = (
        f"Successfully merged cells in range '{range_name}' ({merge_type_normalized}) "
...
async def unmerge_cells(
    ...
    """
    Unmerges previously merged cells in a range.

    Args:
        user_google_email (str): The user's Google email address. Required.
        spreadsheet_id (str): The ID of the spreadsheet. Required.
        range_name (str): The A1 range to unmerge. Required.

    Returns:
        str: Confirmation message of the unmerge operation.
    """
    logger.info(
...
async def protect_sheet_range(
    ...
    """
    Protects or unprotects a range in a sheet to prevent editing.

    Args:
        user_google_email (str): The user's Google email address. Required.
        spreadsheet_id (str): The ID of the spreadsheet. Required.
        range_name (str): The A1 range to protect. Required for "add" action.
        action (str): The action to perform: "add", "update", or "delete". Defaults to "add".
        protected_range_id (int): The ID of the protected range. Required for "update" and "delete".
        description (str): Description of what is protected. Used for "add" and "update".
        warning_only (bool): If True, shows a warning instead of preventing edits. Defaults to False.
        editors (list): List of email addresses that can edit the protected range. Used for "add" and "update".

    Returns:
        str: Confirmation message of the protect operation.
    """
    allowed_actions = {"add", "update", "delete"}
    action_normalized = action.strip().lower()
    if action_normalized not in allowed_actions:
    ...
    logger.info(
        f"[protect_sheet_range] Invoked. Action: '{action_normalized}', Email: '{user_google_email}', "
    ...
    if action_normalized == "add":
        ...
        if editors:
            protected_range["editors"] = {"values": [{"email": e} for e in editors]}
        ...
        text_output = (
            f"Successfully added protection to range '{range_name}' in spreadsheet "
            f"{spreadsheet_id} for {user_google_email}. "
            f"Warning only: {warning_only}, Editors: {len(editors) if editors else 0}."
        )

    elif action_normalized == "update":
        if protected_range_id is None:
            raise UserInputError("protected_range_id is required for action 'update'.")

        existing = None
        for pr in metadata.get("protectedRanges", []):
            if pr.get("protectedRangeId") == protected_range_id:
                existing = pr
                break

        if not existing:
            raise UserInputError(
                f"Protected range with ID {protected_range_id} not found."
            )

        updates = {"protectedRangeId": protected_range_id}

        if description is not None:
            updates["description"] = description
        if warning_only is not None:
            updates["warningOnly"] = warning_only
        if editors is not None:
            updates["editors"] = {"values": [{"email": e} for e in editors]}

        request_body = {
            "requests": [
                {
                    "updateProtectedRange": {
                        "protectedRange": updates,
                        "fields": "description,warningOnly,editors",
                    }
                }
            ]
        }
        ...
        text_output = (
            f"Successfully updated protected range ID {protected_range_id} in spreadsheet "
            f"{spreadsheet_id} for {user_google_email}."
        )
        ...
        text_output = (
            f"Successfully deleted protected range ID {protected_range_id} from spreadsheet "
            f"{spreadsheet_id} for {user_google_email}."
        )

    logger.info(
        f"Successfully performed {action_normalized} protection for {user_google_email}."
    )
    return text_output
