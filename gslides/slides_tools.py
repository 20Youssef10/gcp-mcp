...
async def copy_slide(
    ...
    """
    Duplicates a slide within a presentation.

    Args:
        user_google_email (str): The user's Google email address. Required.
        presentation_id (str): The ID of the presentation. Required.
        slide_id (str): The ID of the slide to duplicate. Required.

    Returns:
        str: Confirmation message with the new slide ID.
    """
    logger.info(
        f"[copy_slide] Invoked. Email: '{user_google_email}', Presentation: '{presentation_id}', Slide: '{slide_id}'"
    )
    ...
    if not source_slide:
        raise UserInputError(f"Slide with ID '{slide_id}' not found in presentation.")

    new_object_id = f"slide_{asyncio.get_event_loop().time():.0f}"
    ...
    if source_index >= 0:
        requests.append(
            {
                "updateSlidesOrder": {
                    "slideObjectIds": [new_object_id],
                    "operation": "INSERT_BEFORE",
                    "targetSlideObjectId": slides[source_index + 1].get("objectId")
                    if source_index + 1 < len(slides)
                    else slides[-1].get("objectId"),
                }
            }
        )
    ...
    confirmation_message = f"""Slide Duplicated Successfully for {user_google_email}:
- Presentation ID: {presentation_id}
- Source Slide ID: {slide_id}
- New Slide ID: {new_object_id}

The duplicated slide has been inserted after the original slide."""
...
async def delete_slide(
    ...
    Args:
        user_google_email (str): The user's Google email address. Required.
        presentation_id (str): The ID of the presentation. Required.
        slide_id (str): The ID of the slide to delete. Required.

    Returns:
        str: Confirmation message of the deletion.
    """
    logger.info(
        f"[delete_slide] Invoked. Email: '{user_google_email}', Presentation: '{presentation_id}', Slide: '{slide_id}'"
    )
    ...
    slides = presentation.get("slides", [])
    slide_exists = any(slide.get("objectId") == slide_id for slide in slides)
    ...
    confirmation_message = f"""Slide Deleted Successfully for {user_google_email}:
- Presentation ID: {presentation_id}
- Deleted Slide ID: {slide_id}"""
...
async def reorder_slides(
    ...
    """
    Reorders slides in a presentation.

    Args:
        user_google_email (str): The user's Google email address. Required.
        presentation_id (str): The ID of the presentation. Required.
        slide_ids (List[str]): List of slide IDs in the desired order. Required.

    Returns:
        str: Confirmation message with the new order.
    """
    logger.info(
        f"[reorder_slides] Invoked. Email: '{user_google_email}', Presentation: '{presentation_id}'"
    )

    if not slide_ids:
        raise UserInputError("At least one slide_id must be provided.")
    ...
    for sid in slide_ids:
        if sid not in existing_slides:
            raise UserInputError(f"Slide with ID '{sid}' not found in presentation.")
    ...
    confirmation_message = f"""Slides Reordered Successfully for {user_google_email}:
- Presentation ID: {presentation_id}
- New Order: {", ".join(slide_ids)}"""
