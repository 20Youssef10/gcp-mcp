...
@server.tool()
@handle_http_errors("move_gmail_message", service_type="gmail")
@require_google_service("gmail", "gmail_modify")
async def move_gmail_message(
    service,
    user_google_email: str,
    message_id: str,
    add_label_ids: Optional[List[str]] = None,
    ...
    """
    Moves a Gmail message by adding and removing labels. This is commonly used to move
    messages to specific folders (like INBOX, Sent, Trash, Spam, or custom labels).

    Args:
        user_google_email (str): The user's Google email address. Required.
        message_id (str): The ID of the message to move. Required.
        add_label_ids (Optional[List[str]]): List of label IDs to add (e.g., "INBOX", "SPAM",
            "TRASH", or custom label IDs like "Label_123"). Required unless removing labels.
        remove_label_ids (Optional[List[str]]): List of label IDs to remove. Required unless adding labels.

    Returns:
        str: Confirmation message with the label changes.
    """
    logger.info(
        f"[move_gmail_message] Invoked. Email: '{user_google_email}', Message ID: '{message_id}'"
    )
    ...
    updated_message = await asyncio.to_thread(
        service.users().messages().modify(userId="me", body=body).execute
    )

    new_labels = updated_message.get("labelIds", [])
    ...
    text_output = (
        f"Successfully moved message '{message_id}' for {user_google_email}. "
        f"{'; '.join(actions)}. "
        f"Current labels: {', '.join(new_labels)}"
    )

    logger.info(f"Successfully moved message for {user_google_email}.")
    return text_output


@server.tool()
@handle_http_errors("send_gmail_reply", service_type="gmail")
...
async def send_gmail_reply(
    ...
    Args:
        user_google_email (str): The user's Google email address. Required.
        thread_id (str): The ID of the thread to reply to. Required.
        body (str): The message body (plain text or HTML). Required.
        to (Optional[str]): The recipient email address. If not provided, uses the sender of the last message.
        cc (Optional[List[str]]): List of CC recipients.
        bcc (Optional[List[str]]): List of BCC recipients.
        subject (Optional[str]): Subject line. If not provided, uses "Re: [original subject]".
        attachments (Optional[List[str]]): List of file paths to attach.
        from_email (Optional[str]): The sender email address (must be one of user's aliases).

    Returns:
        str: Confirmation message with the sent message ID.
    """
    logger.info(
        f"[send_gmail_reply] Invoked. Email: '{user_google_email}', Thread ID: '{thread_id}'"
    )

    thread = await asyncio.to_thread(
        service.users().threads().get(id=thread_id, userId="me").execute
    )

    messages = thread.get("messages", [])
    if not messages:
        raise UserInputError(f"No messages found in thread '{thread_id}'.")
    ...
    original_subject = get_header("subject")
    original_message_id = get_header("message-id")
    ...
    if not to:
        ...
        if not to:
            raise UserInputError(
                "Could not determine recipient. Please provide 'to' parameter."
            )
    ...
    if from_email:
        msg["From"] = formataddr((user_google_email.split("@")[0], from_email))
    else:
        msg["From"] = user_google_email
    ...
    sent_message = await asyncio.to_thread(
        ...
        .send(
            userId="me",
            body={"raw": encoded_msg, "threadId": thread_id},
        )
        .execute
    )
    ...
    text_output = (
        f"Successfully sent reply to thread '{thread_id}' for {user_google_email}. "
        f"Message ID: {message_id}"
    )

    logger.info(
        f"Successfully sent reply for {user_google_email}. Message ID: {message_id}"
    )
    return text_output


@server.tool()
@handle_http_errors("mark_gmail_read", service_type="gmail")
@require_google_service("gmail", "gmail_modify")
async def mark_gmail_read(
    service,
    user_google_email: str,
    message_ids: List[str],
    read: bool = True,
    ...
    """
    Marks one or more Gmail messages as read or unread.

    Args:
        user_google_email (str): The user's Google email address. Required.
        message_ids (List[str]): List of message IDs to mark as read/unread. Required.
        read (bool): If True, marks messages as read. If False, marks as unread. Defaults to True.

    Returns:
        str: Confirmation message of the operation.
    """
    logger.info(
        f"[mark_gmail_read] Invoked. Email: '{user_google_email}', Count: {len(message_ids)}, Read: {read}"
    )

    if not message_ids:
        raise UserInputError("At least one message_id must be provided.")
    ...
    action = "marked as read" if read else "marked as unread"
    text_output = (
        f"Successfully {action} {len(message_ids)} message(s) for {user_google_email}."
    )

    logger.info(f"Successfully {action} messages for {user_google_email}.")
    return text_output
