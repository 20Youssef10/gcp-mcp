...
@server.tool()
@handle_http_errors("quick_add_event", service_type="calendar")
@require_google_service("calendar", "calendar_write")
async def quick_add_event(
    ...
    """
    Creates an event from natural language text using Google's quick add feature.

    Args:
        user_google_email (str): The user's Google email address. Required.
        text (str): The natural language text for the event (e.g., "Lunch with John at noon tomorrow",
            "Team meeting every Monday at 2pm", "Dentist appointment next Friday at 3:30pm"). Required.
        calendar_id (str): The calendar ID. Defaults to "primary".

    Returns:
        str: Confirmation message with the created event details.
    """
    logger.info(
        f"[quick_add_event] Invoked. Email: '{user_google_email}', Text: '{text}', Calendar: {calendar_id}"
    )
    ...
    output_parts = [
        f"Successfully created event from quick add for {user_google_email}:",
        f"  Title: {summary}",
        f"  Start: {start_str}",
    ]
    if end_str:
        output_parts.append(f"  End: {end_str}")
    if location:
        output_parts.append(f"  Location: {location}")
    if description:
        output_parts.append(f"  Description: {description}")

    output_parts.append(f"  Event ID: {event_id}")

    result_text = "\n".join(output_parts)
    logger.info(f"[quick_add_event] Successfully created event: {event_id}")
    return result_text


@server.tool()
@handle_http_errors("watch_calendar", service_type="calendar")
@require_google_service("calendar", "calendar_write")
async def watch_calendar(
    ...
    """
    Sets up push notifications (watch) on a calendar for changes.

    Args:
        user_google_email (str): The user's Google email address. Required.
        calendar_id (str): The calendar ID to watch. Defaults to "primary".
        webhook_url (str): The URL to receive notifications. Required.
        expiration (int): When the watch should expire, in seconds from now. Defaults to 604800 (7 days).
            Maximum is 604800 (7 days).

    Returns:
        str: Confirmation message with channel details.
    """
    logger.info(
        f"[watch_calendar] Invoked. Email: '{user_google_email}', Calendar: {calendar_id}, "
        f"Webhook: {webhook_url}"
    )

    if not webhook_url:
        raise UserInputError("webhook_url is required.")

    if expiration < 60 or expiration > 604800:
        raise UserInputError(
            "expiration must be between 60 and 604800 seconds (1 minute to 7 days)."
        )
    ...
    expiration_iso = expiration_time.strftime("%Y-%m-%dT%H:%M:%SZ")

    body = {
        "id": channel_id,
        "type": "web_hook",
        "address": webhook_url,
        "expiration": int(expiration_iso.replace("-", "").replace(":", "")[:14]),
    }
    ...
    try:
        ...
        if e.resp.status == 409:
            raise UserInputError(
                "A watch is already active for this calendar. Stop the existing watch first before creating a new one."
            )
        raise

    channel_id_response = channel.get("id", channel_id)
    resource_id = channel.get("resourceId", "Unknown")

    output_parts = [
        f"Successfully set up push notifications for {user_google_email}:",
        f"  Calendar: {calendar_id}",
        f"  Webhook URL: {webhook_url}",
        f"  Channel ID: {channel_id_response}",
        f"  Resource ID: {resource_id}",
        f"  Expires: {expiration_iso}",
        "",
        "Note: The watch will expire after 7 days. You need to renew it periodically.",
    ]

    result_text = "\n".join(output_parts)
    logger.info(
        f"[watch_calendar] Successfully set up watch for calendar: {calendar_id}"
    )
    return result_text


@server.tool()
@handle_http_errors("stop_calendar_watch", service_type="calendar")
@require_google_service("calendar", "calendar_write")
async def stop_calendar_watch(
    ...
    """
    Stops push notifications (watch) on a calendar.

    Args:
        user_google_email (str): The user's Google email address. Required.
        calendar_id (str): The calendar ID. Defaults to "primary".
        channel_id (str): The channel ID to stop. If not provided, stops all watches on the calendar.
        resource_id (str): The resource ID (required if channel_id is provided).

    Returns:
        str: Confirmation message.
    """
    logger.info(
        f"[stop_calendar_watch] Invoked. Email: '{user_google_email}', Calendar: {calendar_id}"
    )

    if channel_id and not resource_id:
        raise UserInputError("resource_id is required when channel_id is provided.")
    ...
    if channel_id and resource_id:
        ...
        result_text = f"Successfully stopped watch with channel ID '{channel_id}' for {user_google_email}."
    else:
        result_text = (
            f"Note: To stop a specific watch, provide both channel_id and resource_id. "
            f"To find active watches, check your application's watch registration. "
            f"For {user_google_email}, no specific watch was stopped."
        )

    logger.info(f"[stop_calendar_watch] Completed for {user_google_email}")
    return result_text
