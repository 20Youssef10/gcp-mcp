...
@server.tool()
@handle_http_errors("copy_doc", service_type="docs")
...
async def copy_doc(
    ...
    """
    Copies an existing Google Doc to create a new document.

    Args:
        user_google_email (str): The user's Google email address. Required.
        document_id (str): The ID of the document to copy. Required.
        title (str): The title for the new copied document. Required.

    Returns:
        str: Confirmation message with the new document ID and URL.
    """
    logger.info(
        f"[copy_doc] Invoked. Email: '{user_google_email}', Source: {document_id}, New title: {title}"
    )
    ...
    text_output = (
        f"Successfully copied document to '{title}' for {user_google_email}. "
        f"ID: {new_id} | URL: {new_url}"
    )

    logger.info(
        f"Successfully copied document for {user_google_email}. New ID: {new_id}"
    )
    return text_output


@server.tool()
@handle_http_errors("delete_doc", service_type="docs")
@require_google_service("drive", "drive_write")
async def delete_doc(
    ...
    """
    Deletes or trashes a Google Doc.

    Args:
        user_google_email (str): The user's Google email address. Required.
        document_id (str): The ID of the document to delete. Required.
        trash (bool): If True, moves the document to trash. If False, permanently deletes it.
            Defaults to True. Note: Permanently deleted documents cannot be recovered.

    Returns:
        str: Confirmation message of the deletion.
    """
    logger.info(
        f"[delete_doc] Invoked. Email: '{user_google_email}', Document: {document_id}, Trash: {trash}"
    )
    ...
    if trash:
        if is_trashed:
            raise UserInputError(
                f"Document '{file_name}' (ID: {document_id}) is already in trash."
            )
    ...
    text_output = f"Successfully {action} document '{file_name}' (ID: {document_id}) for {user_google_email}."

    logger.info(f"Successfully {action} document for {user_google_email}.")
    return text_output


@server.tool()
@handle_http_errors("share_doc", service_type="docs")
@require_google_service("drive", "drive_write")
async def share_doc(
    service,
    user_google_email: str,
    ...
    """
    Shares a Google Doc with another user or group.

    Args:
        user_google_email (str): The user's Google email address. Required.
        document_id (str): The ID of the document to share. Required.
        email (str): The email address to share with. Required.
        role (str): The role to grant. Options: "reader", "commenter", "writer". Defaults to "writer".
        type (str): The type of recipient. Options: "user", "group", "domain". Defaults to "user".
        notify (bool): Whether to send email notifications. Defaults to True.
        message (Optional[str]): Custom message to include in the notification email.

    Returns:
        str: Confirmation message of the share operation.
    """
    logger.info(
        f"[share_doc] Invoked. Email: '{user_google_email}', Document: {document_id}, "
        f"To: {email}, Role: {role}, Type: {type}"
    )

    valid_roles = {"reader", "commenter", "writer"}
    if role not in valid_roles:
        raise UserInputError(f"role must be one of {valid_roles}, got '{role}'.")

    valid_types = {"user", "group", "domain"}
    if type not in valid_types:
        raise UserInputError(f"type must be one of {valid_types}, got '{type}'.")
    ...
    if notify and type == "user":
        permission_body["notify"] = True
        if message:
            permission_body["emailMessage"] = message
    ...
    permission_type = permission.get("type", type)
    permission_role = permission.get("role", role)

    text_output = (
        f"Successfully shared document '{file_name}' (ID: {document_id}) for {user_google_email}. "
        f"Shared with: {email} ({permission_type}) as {permission_role}. "
        f"Notification sent: {notify}"
    )

    logger.info(f"Successfully shared document for {user_google_email}.")
    return text_output


@server.tool()
@handle_http_errors("get_doc_permissions", service_type="docs")
@require_google_service("drive", "drive_read")
async def get_doc_permissions(
    ...
    """
    Gets the sharing permissions for a Google Doc.
    ...
    Returns:
        str: Formatted list of permissions for the document.
    """
    logger.info(
        f"[get_doc_permissions] Invoked. Email: '{user_google_email}', Document: {document_id}"
    )
    ...
    file_metadata = await asyncio.to_thread(
        ...
        .get(
            fileId=document_id,
            fields="name,webViewLink,owners,permissions",
            supportsAllDrives=True,
        )
        .execute
    )
    ...
    output_parts = [
        f"Permissions for '{file_name}' (ID: {document_id}):",
        f"Link: {web_link}",
        "",
    ]

    if owners:
        output_parts.append("Owners:")
        for owner in owners:
            email = owner.get("emailAddress", "Unknown")
            display_name = owner.get("displayName", "")
            output_parts.append(
                f"  - {email}" + (f" ({display_name})" if display_name else "")
            )
        output_parts.append("")
    ...
    if permissions:
        ...
        for perm in permissions:
            p_type = perm.get("type", "Unknown")
            p_role = perm.get("role", "Unknown")
            email = perm.get("emailAddress", "")
            domain = perm.get("domain", "")

            if p_type == "user" and email:
                output_parts.append(f"  - {email}: {p_role}")
            elif p_type == "group" and email:
                output_parts.append(f"  - Group {email}: {p_role}")
            elif p_type == "domain" and domain:
                output_parts.append(f"  - Domain {domain}: {p_role}")
            elif p_type == "anyone":
                output_parts.append(f"  - Anyone: {p_role}")
            else:
                output_parts.append(f"  - {p_type}: {p_role}")
    else:
        output_parts.append("No additional permissions (owner-only access).")
